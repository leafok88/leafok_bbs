<?php
	...
	$sql = "UPDATE bbs SET ontop = $set WHERE AID = $id";

	$rs = mysqli_query($db_conn, $sql);
	if ($rs == false)
	{
		$result_set["return"]["code"] = -2;
		$result_set["return"]["message"] = "Set ontop error: " . mysqli_error($db_conn);
	...
	//Add log
	$rs = article_op_log($id, $_SESSION["BBS_uid"], ($set ? "F" : "V"), client_addr(), $db_conn);
	if ($rs == false)
	{
		$result_set["return"]["code"] = -2;
